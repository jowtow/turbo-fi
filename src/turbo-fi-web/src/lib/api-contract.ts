...
export type GetResponse<Path extends GetPath> = Path extends "/auth/me"
  ? CurrentUser
  : Path extends "/accounts"
    ? Account[]
    : Path extends "/expense-types"
      ? ExpenseType[]
      : Path extends "/categories"
        ? Category[]
...
export type PostBody<Path extends PostPath> = Path extends "/auth/register"
  ? RegisterRequest
  : Path extends "/auth/login"
    ? Credentials
    : Path extends "/auth/logout"
      ? undefined
      : Path extends "/accounts"
        ? AccountRequest
        : Path extends "/expense-types"
...
export type PostResponse<Path extends PostPath> = Path extends "/accounts"
  ? Account
  : Path extends "/expense-types"
    ? ExpenseType
    : Path extends "/categories"
...
export type PutBody<Path extends PutPath> =
  Path extends `/transactions/${string}/category`
    ? CategorizeTransactionRequest
    : Path extends `/expense-types/${string}`
      ? ExpenseTypeRequest
      : Path extends `/categories/${string}`
        ? CategoryRequest
        : Path extends `/planned-entries/${string}`
