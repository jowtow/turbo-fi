...
function suggestedCategoryName(transaction: ReviewTransaction, categories: Category[]): string {
  if (!transaction.suggestedCategoryId) return '—'
  return categories.find(c => c.id === transaction.suggestedCategoryId)?.name ?? '—'
}

export function CategorizeWorkspace() {
  const queryClient = useQueryClient()
  const { accounts, expenseTypes, categories, phraseRules } = useFinanceReferenceData(true)
  ...
  return <div className="workspace">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <CategorizationCard accounts={accounts} expenseTypes={expenseTypes} categories={categories} phraseRules={phraseRules} transactions={transactions} onChanged={refresh} />
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {transactions.map((transaction, index) => (
              <tr key={transaction.id} className={`border-b border-emerald-900 ${index === 0 ? 'bg-emerald-900/40' : ''}`}>
                <td className="py-2 pr-6 text-emerald-300">{transaction.transactionDate}</td>
                <td className="py-2 pr-6 max-w-xs truncate">{index === 0 && <span className="mr-2 rounded-full bg-lime-700 px-2 py-0.5 text-xs text-lime-100">Current</span>}{transaction.description}</td>
                <td className={`py-2 pr-6 text-right tabular-nums ${transaction.amount < 0 ? 'text-lime-300' : ''}`}>{money(transaction.amount)}</td>
                <td className="py-2 text-emerald-200">{suggestedCategoryName(transaction, categories)}</td>
              </tr>
            ))}
          </tbody>
        </table>
